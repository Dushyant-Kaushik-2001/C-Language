...
3. Since while(condition) is written at the end of the statement, it is followed by a semicolon as a mark of 
termination of the statement. 
4. Control when reaches ‘do’ keyword, it simply enters into the do-while block. Where it executes statements and 
then reaches to the end of block. Just after the block while(condition) is encountered. Where condition is 
evaluated either as true or false. If it is true, control moves back to the keyword ‘do’ and repetition begins. 
If condition is false, it simply move forward to the next statement of the program.
5. In do while loop, even if the condition is false for the first time, then also control is able to visit do 
while block once. This is why it is called exit control loop. 
